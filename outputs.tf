...
output "all_subnets_by_name" {
  description = "A map of the subnet name to the subnet ID i.e. private/us-east-1a => subnet_id"
  value       = local.all_subnets_by_name
}
...
output "natgw_id_per_az" {
  ...
  value       = local.enable_nat_gateway ? module.vpc.natgw_id_per_az : null
}
...
output "nat_public_ips" {
  ...
  value       = local.enable_nat_gateway ? [] : [for x in module.vpc.nat_gateway_attributes_by_az : x.public_ip]
}
